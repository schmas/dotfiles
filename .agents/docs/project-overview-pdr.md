...
- `README.md` and the `.agents/docs/` tree are repo-only references that never
  land in `$HOME`. `README.md` is excluded by `home/.chezmoiignore`; `.agents/`
  sits outside the chezmoi source root (`home/`), so it is never a candidate for
...
- **External** — an artifact fetched at apply time rather than stored in the
  repo. Declared in `home/.chezmoiexternal.toml`, which currently declares none.
