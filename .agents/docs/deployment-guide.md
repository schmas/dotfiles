...
`upall` (the Go update TUI) is installed once from its GitHub release by
`home/.chezmoiscripts/01-common/run_once_after_05.5-upall-install.sh.tmpl`, into
`~/.local/bin/upall`. The script skips the download when the binary is already
executable, and `upall` self-updates after that — there is no chezmoi external to
refresh. The `upall-classic` bash fallback ships in `home/bin/`.
