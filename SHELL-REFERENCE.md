...
This document provides a comprehensive reference for all aliases, functions, and scripts defined in this dotfiles repository, organized by shell.
...
Fish shell uses abbreviations instead of aliases. Abbreviations expand as you type them, showing the full command before execution.
...
| `nix-config-up` | `git -C ~/.config/nix-config pull && darwin-rebuild switch --flake ~/.config/nix-config#{$hostname}` | Update Nix config |
